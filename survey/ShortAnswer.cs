...
namespace survey
{
    public partial class ShortAnswer : Form
    {
        ...
        private void ShortAnswer_FormClosed(object sender, FormClosedEventArgs e)
        {
            Application.Exit();
        }
        ...
        public void SAQSave()
        {
            ...
            Login.db.Open();
            string query = "insert into ShortAnswer (Question, CategoryId, TypeId, GroupId) values ('" + question + "', " + category + ", " + type + ", " + grp + ")";
            SqlCommand sqlCommand = new SqlCommand(query, Login.db);
            if (question != "" &&  survey_category_combobox.SelectedIndex != -1)
            {
