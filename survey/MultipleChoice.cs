...
namespace survey
{
    ...
    public partial class MultipleChoice : Form
    {
        ...
        private void SingleMultipleChoice_FormClosed(object sender, FormClosedEventArgs e)
        {
            Application.Exit();
        }
        private void SingleMultipleChoice_Load(object sender, EventArgs e)
        {
        ...
        {
            SurveyEdit.typeid = 1;
            SurveyEdit.categoryid = Convert.ToInt32(survey_category_combobox.SelectedValue);
            this.Hide();
        ...
        public void MCQSave()
        {
            ...
            while (reader.Read())
            {
                ...
                SurveyEdit.groupid = Convert.ToInt32(reader["Id"]);
            }
            ...
            Login.db.Close();
            Login.db.Open();
            string query = "insert into MultipleChoice (Question, AnswerA, AnswerB, AnswerC, AnswerD, CategoryId, TypeId, GroupId) values ('" + question + "', '" + answerA + "', '" + answerB + "', '" + answerC + "', '" + answerD + "', " + category + ", " + type + ", " + grp + ")";
            SqlCommand sqlCommand = new SqlCommand(query, Login.db);
            if (question != "" && answerA != "" && answerB != "" && answerC != "" && answerD != "" && survey_category_combobox.SelectedIndex != -1)
            {
